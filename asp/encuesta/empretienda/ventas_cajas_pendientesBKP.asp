...
' -------------------------
Do While Not rs.EOF
    itemFechaRaw = GetFieldValue(rs, "FechaEntrega")
    itemTipo = SafeCStr(GetFieldValue(rs, "TipoCaja"))
    tmpObs = SafeCStr(GetFieldValue(rs, "Observaciones"))
    
    ' TotalCajas
    Dim totalCajasVal
    totalCajasVal = GetFieldValue(rs, "TotalCajas")
    ...
    ' TotalClientes
    totalClientesVal = GetFieldValue(rs, "TotalClientes")
    If IsNumeric(totalClientesVal) Then totalClientesVal = CLng(totalClientesVal) Else totalClientesVal = 0

    ' Clientes
    clientesVal = SafeCStr(GetFieldValue(rs, "Clientes"))

    ' Normalizar fecha
    If itemFechaRaw <> "" And IsDate(itemFechaRaw) Then
		itemFecha = Right("0" & Day(CDate(itemFechaRaw)),2) & "-" & Right("0" & Month(CDate(itemFechaRaw)),2) & "-" & Year(CDate(itemFechaRaw))
    ElseIf itemFechaRaw <> "" Then
        ...
        <table>
            ...
            <tfoot>
                <tr style="background:#f7f7f7; font-weight:700;">
                    <td colspan="2">Totales por Tipo</td>
                    <td colspan="4">
                        <%
                        If dictTotalesPorTipo.Count > 0 Then
                            For Each k In dictTotalesPorTipo.Keys
                                Response.Write Server.HTMLEncode(k) & ": " & dictTotalesPorTipo(k) & "<br />"
                            Next
                        Else
                            Response.Write "- "
                        End If
                ...
                <tr style="background:#e9ecef; font-weight:800;">
                    <td colspan="2">TOTAL GENERAL (Cajas)</td>
                    <td style="text-align:right;"><%= totalGeneral %></td>
                    <td colspan="3">
                        <%
                        Dim sumTotalClientes
                        sumTotalClientes = 0
                        For Each k In dictTotalClientesPorFecha.Keys
                            If IsNumeric(dictTotalClientesPorFecha(k)) Then
                                sumTotalClientes = sumTotalClientes + CLng(dictTotalClientesPorFecha(k))
                            End If
                        Next
                        Response.Write "TOTAL CLIENTES (sum por fecha): " & sumTotalClientes
                        %>
                    </td>
                </tr>
            </tfoot>
...
<div id="obsModal" style="display:none;">
    <div id="obsModalContent">
        <span id="closeModal" style="position:absolute; right:14px; top:10px; cursor:pointer;">&times;</span>
        <h3>Observaciones</h3>
