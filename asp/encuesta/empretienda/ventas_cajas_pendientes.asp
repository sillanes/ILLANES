...
' ===========================
Dim fechaKey, safeObsJS, safeClientesJS
dataJson = "["
For Each fechaKey In arrFechasDict.Keys
    safeObsJS = Replace(Replace(Server.HTMLEncode(arrFechasDict(fechaKey)("Observaciones")), """", "\""") , vbCrLf, "\n")
    safeClientesJS = Replace(Replace(Server.HTMLEncode(arrFechasDict(fechaKey)("Clientes")), """", "\""") , vbCrLf, "\n")
    dataJson = dataJson & "{""fecha"":""" & Replace(fechaKey,"""","'") & """,""tipos"":""" & Replace(arrFechasDict(fechaKey)("Tipos"),"""","'") & """,""cantidadTotal"":" & arrFechasDict(fechaKey)("CantidadTotal") & ",""obs"":""" & safeObsJS & """,""totalClientes"":" & arrFechasDict(fechaKey)("TotalClientes") & ",""clientes"":""" & safeClientesJS & """},"
Next
If Right(dataJson,1) = "," Then dataJson = Left(dataJson, Len(dataJson)-1)
dataJson = dataJson & "]"
...
const datasets = tipos.map(function(tipo, idx){
    return {
        ...
        data: labels.map(function(fecha){
            // Filtramos todos los registros que tengan esa fecha
            const registros = dataCajas.filter(d => d.fecha === fecha);
            let total = 0;
            registros.forEach(r => {
                const regex = new RegExp(tipo + ":\\s*(\\d+)");
                const match = r.tipos.match(regex);
