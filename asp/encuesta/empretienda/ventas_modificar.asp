...
<div class="main-content">
    ...
    <div class="header">
        ...
        <a href="ventas_detalle.asp?FechaInicio=<%=toISO(FechaInicio)%>&FechaFin=<%=toISO(FechaFin)%>&order=<%=orderBy%>&dir=<%=orderDir%>" class="btn-volver"><i class="fas fa-arrow-left"></i> Volver</a>
    </div>

    <div class="form-container">
        <form method="post" action="ventas_modificar_guardar.asp">
			...
			<div class="form-group">
				...
				<select name="VendedorID" style="width:100%; padding:8px; border-radius:6px; border:1px solid #ddd;">
					<% Do While Not rsVendedores.EOF %>
						<option value="<%=rsVendedores("VendedorID")%>" <% If cstr(rsVendedores("VendedorID")) = cstr(rs("VendedorID")) Then Response.Write "selected" %>>
							<%=rsVendedores("Nombre")%> 
						</option>
			...
			<div class="form-group">
                <label>Zona <i class="fas fa-check" title="Modificable"></i></label>
                <textarea id="myTextArea" name="Zona" rows="1"><%=rs("Zona")%></textarea>
            </div>			
			
            <div class="form-group">
                <label>Observaciones Adicional <i class="fas fa-check" title="Modificable"></i></label>
							
				<div class="textarea-container">
					<textarea id="myTextArea" name="ObservacionesAdicional" rows="4"><%=rs("ObservacionADM")%></textarea>
				</div>
                ...
                <table>
                    ...
                    <% Do While Not rsCajas.EOF %>
                        <tr>
                            ...
                            <td><input type="checkbox" name="CajaMod_<%=rsCajas("CajaID")%>" checked></td>
							<% Else %>
                            <td><input type="checkbox" name="CajaMod_<%=rsCajas("CajaID")%>"></td>
							<% End If %>
                        </tr>
                        <% rsCajas.MoveNext
                    Loop %>
            ...
            <div class="form-actions">
                ...
                <a href="ventas_detalle.asp?FechaInicio=<%=toISO(FechaInicio)%>&FechaFin=<%=toISO(FechaFin)%>&order=<%=orderBy%>&dir=<%=orderDir%>" class="btn btn-secondary"><i class="fas fa-times"></i> Cancelar</a>
            </div>
        </form>
    </div>
</div>
