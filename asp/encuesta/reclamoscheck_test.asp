...
Dim Form: Set Form = New ASPForm 
  ...
  Comment = "document.getElementsByName('txtresolucion')[0].value"

  HTML = HTML & hResult
  ' HTML = HTML & "Archivo subido a '" & rel_Folder & "' folder. Limit:" & Form.SizeLimit/1024 & "kB.<br>"
  HTML = HTML & "<form name=""FFUP"" method=post ENCTYPE=multipart/form-data Action=" & ref("UploadID=" & UploadID & "&Action=UPLOAD&doWhat=1&idReclamo="&idReclamo&"&txtresolucion="&Comment) & " OnSubmit=""return ProgressBar();"">"
  ' HTML = HTML & "<span  class=""pagetitle""><span>Agregar documento</span><Input Type=Button Class=""btn1"" Value=""+"" OnClick=Expand()> </span>"
  HTML = HTML & "</br>"
...
<body>
	...
	<form name="FF" method="post" action="reclamoscheck_test.asp">
...
<%
	...
	<td>
		<span  class="pagetitle">
			...
			<textarea id="textarea1" name="txtresolucion" style="overflow:auto;resize:none" wrap="hard" rows="12" cols="20" maxlength="500">Estimado <%=dbRS("NombreCliente")%>:</textarea>

<%=do_Upload("C:\Reclamos\Archivos\")%>
...
<!--#INCLUDE FILE="_common.asp"--> 
	...
	<form name="FF2" method="post" action="reclamoscheck_test.asp">	

	<input type="hidden" name="doWhat" value="<%=doWhat%>">
	<input type="hidden" name="idReclamo" value="<%=idReclamo%>">  
	<input type="hidden" name="txtresolucion" value="<%=txtresol%>">  
	
	<div align="center">
		<br/>
		<input type="button" class="btn1" value="Volver" onClick="resetForm(this.form)">
		<input type="button" class="btn1" value="Cerrar Reclamo" onClick="chkForm3(this.form, 2,'<%=idReclamo%>')">		
	</div>
...
function chkForm3(Fm,prm,param1){ 
	var msg  = ""; 
	if(msg != "") alert(msg);
	else{ 
		Fm.doWhat.value = prm; 
