...
sql = "EXEC report.usp_Ordenes_Ventas_Detalle_sel '" & toISO(FechaInicio) & "','" & toISO(FechaFin) & "','" & orderBy & "','" & orderDir & "'" & ",'Logistica'"
'response.write sql
...
<div class="header">
    ...
    <div style="display:flex; gap:10px;">
        <a href="ordenes.asp?txtFechaInicio=<%=toISO(FechaInicio)%>&txtFechaFin=<%=toISO(FechaFin)%>" class="btn-volver">
            <i class="fas fa-arrow-left"></i> Volver
        </a>
        <a href="ordenes_excel.asp?FechaInicio=<%=toISO(FechaInicio)%>&FechaFin=<%=toISO(FechaFin)%>&order=<%=orderBy%>&dir=<%=orderDir%>" 
...
<tr class="<%=rowClass%>">
    ...
    <td>
        <a href="ordenes_modificar.asp?FileID=<%=FileID%>&OrdenID=<%=OrdenID%>&FechaInicio=<%=toISO(FechaInicio)%>&FechaFin=<%=toISO(FechaFin)%>" class="icon-btn edit" title="Editar"><i class="fas fa-edit"></i></a>
