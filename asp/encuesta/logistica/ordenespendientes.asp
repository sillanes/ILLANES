...
If Request("txtFechaInicio") <> "" Then
    FechaInicio = CDate(Request("txtFechaInicio"))
Else
    FechaInicio = FechaInicioSP
End If

If Request("txtFechaFin") <> "" Then
    FechaFin = CDate(Request("txtFechaFin"))
Else
    FechaFin = Date()
End If
...
' ==========================================
Dim rs, sql
sql = "EXEC report.usp_Ordenes_Ventas_sel '" & toISO(FechaInicio) & "','" & toISO(FechaFin) & "'" & ",'LogisticaPendientes'"
Set rs = conn.Execute(sql)

' ==========================================
' Acumuladores
' ==========================================
Dim totalBronce, totalPlata, totalOro, totalPlatino, totalEspecial, totalImporte, totalEstrella
totalBronce = 0
totalPlata = 0
totalOro = 0
totalPlatino = 0
...
<div class="filter-bar">
    <form method="get" action="ordenes.asp">
        <label>Desde:</label>
        <input type="date" name="txtFechaInicio" value="<%=toISO(FechaInicio)%>">
        <label>Hasta:</label>
        <input type="date" name="txtFechaFin" value="<%=toISO(FechaFin)%>">
...
<tr>
    ...
    <th>Cajas Bronce</th>
    <th>Cajas Estrella</th>
    <th>Pedidos Pendientes De Clasificacion</th>
...
<%
Do While Not rs.EOF
    Dim pedidos, oro, plata, platino, especial, bronce, importe, faltaCobrar, porcentaje,estrella
    pedidos = parseNumber(rs("pedidos"))
    ...
    totalpedidos = parseNumber(rs("pedidos"))
    PedidosPendientesDeClasificacion = parseNumber(rs("PedidosPendientesDeClasificacion"))
	
    PedidosPendientesDeEntrega  = parseNumber(rs("PedidosPendientesDeEntrega"))
	
	If totalpedidos > 0 Then
        porcentaje = ((totalpedidos - PedidosPendientesDeEntrega) / totalpedidos) * 100
    Else
        porcentaje = 0
    End If
...
<tr>
    ...
    <td><%=bronce%></td>
    <td><%=estrella%></td>
    <td><%=PedidosPendientesDeClasificacion%></td>
    <td><%=PedidosPendientesDeEntrega%></td>
	<td>
    ...
    <td>
        <a href="ordenes_pendientes_detalle.asp?FechaInicio=<%=toISO(FechaInicio)%>&FechaFin=<%=toISO(FechaFin)%>">Ver</a>
    </td>
</tr>
<%
    rs.MoveNext
