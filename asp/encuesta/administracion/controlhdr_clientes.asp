...
<head>
    ...
    <style>
		...
		.card p { margin:4px 0; font-size:1em; }
		.card small { font-size:.9em; opacity:.9; }
		.bg-blue { background:#2e30c7; }
		.bg-green { background:#28a745; }
		.bg-red { background:#ff030e; }	
    </style>
    <script>
...
<div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
    <div style="display: flex; align-items: center; gap: 10px;">
        ...
        <a href="controlhdr.asp?fecha=<%=fecha%>" 
           class="icon-btn" 
           title="Volver"
                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th rowspan="2">Detalle</th> <!-- Nueva columna -->
                    ...
                    <tr>
						...
						<td style="background-color: <%= rs("color") %>; text-align:center;">
							<% If rs("CantidadComprobantes") > 0 Then %>
								<a href="comprobantes_cliente.asp?hdrid=<%= hdrid %>&clienteid=<%= rs("ClienteID") %>" 
						...
						<a href="detalle_clientes.asp?hdrid=<%= hdrid %>&clienteid=<%= rs("ClienteID") %>&cond=<%= Server.URLEncode(FormaPago) %>" 
						   title="Ver detalle">
...
Dim ComentarioRecibe, Deposito, DepositoObs, Gastos, GastosObs
 

' Pasar al 2º recordset
Dim rsInfo : Set rsInfo = rs.NextRecordset()

If Not (rsInfo Is Nothing) Then
    If Not rsInfo.EOF Then
        ComentarioRecibe = NzV(rsInfo("RepartoComentarioRecibe"))
        Deposito         = NzN(rsInfo("RepartidorDeposito"))
        DepositoObs      = NzV(rsInfo("RepartidorDepositoObs"))
        Gastos           = NzN(rsInfo("RepartidorGastos"))
        GastosObs        = NzV(rsInfo("RepartidorGastosObs"))
    End If
...
' Helpers para NULL
Function NzV(v) : If IsNull(v) Or v = "" Then NzV = "" Else NzV = v End If : End Function
Function NzN(v) : If IsNull(v) Or v = "" Then NzN = 0 Else NzN = CDbl(v) End If : End Function
 
' Mostrar cards solo si hay algún dato relevante
If (Len(ComentarioRecibe) > 0) Or (Deposito <> 0) Or (Len(DepositoObs) > 0) Or (Gastos <> 0) Or (Len(GastosObs) > 0) Then
%>
  <div class="cards-grid">
    <div class="card bg-blue">
      <h3><i class="fas fa-comment"></i> Comentario recibido</h3>
      <p><%= Server.HTMLEncode(ComentarioRecibe) %>&nbsp;</p>
    </div>

    <div class="card bg-green">
      <h3><i class="fas fa-university"></i> Importe depositado</h3>
      <p style="font-weight:bold;">$<%= FormatNumber(Deposito, 2) %></p>
      <small><em><%= Server.HTMLEncode(DepositoObs) %>&nbsp;</em></small>
    </div>

    <div class="card bg-red">
      <h3><i class="fas fa-receipt"></i> Gastos</h3>
      <p style="font-weight:bold;">$<%= FormatNumber(Gastos, 2) %></p>
      <small><em><%= Server.HTMLEncode(GastosObs) %>&nbsp;</em></small>
    </div>
  </div>
<% End If %>
