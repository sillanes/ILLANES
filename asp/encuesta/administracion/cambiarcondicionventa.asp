...
nuevaCondicion = Request.Form("nuevaCondicion")

If Request.ServerVariables("REQUEST_METHOD") = "POST" And Len(nuevaCondicion) > 0 Then
    sql = "EXEC usp_Transportista_HojaDeRuta_FormaPago_upd " & _
          Request("hdr") & ", " & Request("clienteid") & ", '" & Replace(nuevaCondicion, "'", "''") & "'"
    conn.Execute sql
    Response.Write "<div style='color:green;margin-bottom:10px;'>✅ Condición de venta actualizada correctamente.</div>"
End If
%>
...
<head>
    ...
    <title>Cambiar Condición de Venta</title>
...
<div class="main-content">
	...
	<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 20px;">
		...
		<h2>Cambiar Condición de Venta</h2>
    ...
    <form method="get" action="cambiarcondicionventa.asp" style="margin-bottom:20px;">
...
If HDR <> "" Or ClienteID <> "" Then
    sql = "EXEC usp_Transportista_HojaDeRuta_Buscar " & _
    ...
    If Not rsCliente.EOF Then
        ...
        Dim clienteNombre, formaPagoCliente
        currentHDR = -1
        currentClienteID = -1
        ...
        Do While Not rsCliente.EOF
            If rsCliente("HojaDeRutaID") <> currentHDR Then
                If currentClienteID <> -1 Then
                    ...
                    Call EscribirFormularioCondicion(currentHDR, currentClienteID, formaPagoCliente)
                    Response.Write "</div>"
                End If
...
Sub EscribirFormularioCondicion(HDR, ClienteID, FormaActual)
    Response.Write "<form method='post' action='cambiarcondicionventa_confirmar.asp' class='update-form'>"
    Response.Write "<input type='hidden' name='hdr' value='" & HDR & "'>"
    Response.Write "<input type='hidden' name='clienteid' value='" & ClienteID & "'>"
    Response.Write "<label for='nuevaCondicion_" & ClienteID & "'>Nueva Condición:</label>"
    Response.Write "<select name='nuevaCondicion' id='nuevaCondicion_" & ClienteID & "'>"

    Dim rsCondiciones
    Set rsCondiciones = conn.Execute("EXEC usp_Transportista_HojaDeRuta_Condicion_Combo")
