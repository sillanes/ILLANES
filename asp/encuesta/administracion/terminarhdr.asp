...
Function FormatearImporte(valor)
    Dim valorTexto, valorNum, entero, decimalPart, resultado, i, contador, isNegative

    ' Primero, convertir valor a string, quitar espacios
    valorTexto = Trim(CStr(valor))

    ' Cambiar punto decimal a coma para VBScript (si tiene punto decimal)
    valorTexto = Replace(valorTexto, ".", ",")

    ' Si no es numérico, devolver 0.00
    If Not IsNumeric(valorTexto) Then
        FormatearImporte = "0.00"
        Exit Function
    End If

    valorNum = CDbl(valorTexto)

    isNegative = (valorNum < 0)
    valorNum = Abs(valorNum)

    valorNum = Round(valorNum, 2)

    entero = Int(valorNum)
    decimalPart = Right("00" & CStr(Int((valorNum - entero) * 100 + 0.5)), 2)
    ...
    For i = Len(strEntero) To 1 Step -1
        ...
        If (contador Mod 3 = 0) And (i > 1) Then
            resultado = "," & resultado ' coma como separador de miles
        End If
    Next

    FormatearImporte = IIf(isNegative, "-", "") & resultado & "." & decimalPart
End Function
...
Function FormatearImporte2(valor)

    Dim limpio, entero, decimalPart, resultado, i, contador

	limpio = Replace((valor), ".", "")
	limpio = Replace((valor), ",", ".")
    'limpio = Replace(CStr(valor), ",", "") ' Quitar comas de miles
    'limpio = Replace(limpio, ".", ".") ' Mantener punto decimal
...
<head>
   ...
   <script>
		...
		function formatCurrency(value) {
			...
			let formatted = number.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
		...
		document.addEventListener('DOMContentLoaded', function() {
			document.querySelectorAll('.currency-input').forEach(input => {
				input.addEventListener('blur', function() {
					...
					// Aceptar el guion inicial para negativos
					let isNegative = val.startsWith('-');
					val = val.replace(/[^0-9.]/g, '');

					if (val !== '') {
						let formatted = formatCurrency((isNegative ? '-' : '') + val);
				...
				input.addEventListener('input', function() {
					let val = this.value;
					// Permitir solo un guion al inicio si es negative-allowed
					if (this.classList.contains('negative-allowed')) {
						val = val.replace(/(?!^)-/g, '');
					} else {
						val = val.replace(/-/g, '');
					}
			...
			form.addEventListener('submit', function(e) {
				document.querySelectorAll('.currency-input').forEach(input => {
					let val = input.value.replace(/,/g, ''); // Quitar separadores de miles
					input.value = val;
				});
			});
		}); 
        ...
        <form action="procesar_terminar_hdr.asp" method="post" class="form-layout">
            ...
            <div class="form-row">
               <div class="form-group">
                    ...
                    <div class="input-group">
                        <span class="input-prefix">$</span>
                        <input type="text" name="Diferencia" value="<%=FormatearImporte(Diferencia)%>" class="form-control currency-input negative-allowed">
            ...
            <div class="form-row">
                ...
                <input type="hidden" name="PendientesJustificadas" value="<%=PendientesJustificadas%>" class="form-control">
