...
NroCliente = objRequest("NroRecibo")
...
<body>
	...
	<form name="FF" method="post" action="ordenespendientes.asp">
	<input type="hidden" name="doWhat" value="<%=doWhat%>"> 
	<input type="hidden" name="periodoID" value="<%=periodoid%>">  
	...
	<input type="hidden" name="NroRecibo" value="">
 ...
 <%
	...
	<!--#include file="./includes/OrdenesPendientes.asp" --> 
...
<%
	...
	<!--#include file="./includes/ordenespendientesMostrar.asp" --> 
 	<input type="button" class="btn1" value="Volver Menu" onClick="volvermenu(this.form)" style="width:80px;">
...
<%	
		...
		If IsNumeric(NroCliente) And Len(NroCliente)>=4 And Len(NroCliente)<=8 Then
			sSQL = "EXEC dbo.[usp_Ordenes_Cliente_save] " & _
		...
		Else
			Response.Write "<script>alert('Nº de recibo inválido (4-8 dígitos).');</script>"
		End If 

 
		if hasError=0 then
%>
		<span class="textSuccess">Ordenes: <%=OrdenID%> actualizada</span><br/> 	 
<%
		Else
%>
...
function guardarFila(rowID){
    var input = document.getElementById('NroRecibo_'+rowID);
    var nro   = input.value.trim();
    if(!/^\d{4,8}$/.test(nro)){
        alert('El Nº de Recibo debe tener entre 4 y 8 dígitos.');
        input.focus();
        return;
    }
    var Fm = document.FF;
    Fm.RowID.value      = rowID;
    Fm.NroRecibo.value = nro;
    Fm.doWhat.value     = 3;
    Fm.submit();
}
</script>	  
