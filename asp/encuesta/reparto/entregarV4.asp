...
<%
' ===========================
' 🔧 Función corregida (mantiene separadores correctamente)
' ===========================
...
function actualizarTotales() {
  ...
  document.querySelectorAll('.item-factura').forEach(function (item) {
    const cantActual   = parseFloat(item.querySelector('.item-cantidad').textContent.replace(',', '.'));
    const cantOriginal = parseFloat(item.dataset.cantorig.replace(',', '.'));
    const totalImpOriginal = parseFloat(item.dataset.totalimp);

    let totalItem = totalImpOriginal;
    if (cantOriginal > 0 && cantActual !== cantOriginal) {
      const unit = totalImpOriginal / cantOriginal;
      totalItem = unit * cantActual;
    }

    // comparar valores
    const diff = totalItem - totalImpOriginal;
    ...
    if (Math.abs(diff) > 0.001) {
      ...
      const mensaje = `Factura ${item.dataset.facturaid} | Item ${item.dataset.itemid} | CantOrig: ${cantOriginal} | CantAct: ${cantActual} | Original: ${totalImpOriginal.toFixed(2)} | Recalc: ${totalItem.toFixed(2)} | Δ ${diff.toFixed(2)}`;
      diferencias.push(mensaje);
      ...
      // 🟢 marcar disminución
        ...
        item.style.border = "2px solid #198754";
      }
    }
  ...
  // mostrar resumen visual de diferencias
  let panel = document.getElementById("panelDiferencias");
  if (!panel) {
    panel = document.createElement("div");
    panel.id = "panelDiferencias";
    panel.style.cssText = "margin-top:15px; padding:10px; background:#f8f9fa; border:1px solid #ccc; border-radius:6px; font-size:13px;";
    document.querySelector(".container").appendChild(panel);
  }

  if (diferencias.length > 0) {
    let html = "<strong>⚠️ Se detectaron diferencias en los siguientes ítems:</strong><ul style='margin-top:8px;'>";
    ...
    panel.innerHTML = "<span style='color:green;'>✅ No se detectaron diferencias al recalcular.</span>";
  }
...
function restaurarCantidades() {
  // 🔁 vuelve todo a los valores originales exactos
  document.querySelectorAll('.item-factura').forEach(item => {
    const cantOrig = item.dataset.cantorig;
    const totalImpOriginal = parseFloat(item.dataset.totalimp);

    // restaurar cantidad y total
    item.querySelector('.item-cantidad').textContent = cantOrig;
    item.querySelector('.item-total-imp').textContent = formatNumberAr(totalImpOriginal);
    item.querySelector('.input-cantidad-item').value = cantOrig;
  ...
  // recalcular totales y verificar diferencias visualmente
  actualizarTotales();

  // hacer scroll al panel de diferencias si existen
  const panel = document.getElementById("panelDiferencias");
  if (panel) {
...
function diagnosticarTotales() {
  let totalOriginalHTML = parseFloat(
    document.getElementById("totalGeneral").dataset.totaloriginal || "0"
  );
  let totalSumadoItems = 0;
  let totalPorFactura = {};

  document.querySelectorAll(".item-factura").forEach(item => {
    const facturaID = item.dataset.facturaid;
    const totalItem = parseFloat(item.dataset.totalimp);
    totalSumadoItems += totalItem;
    if (!totalPorFactura[facturaID]) totalPorFactura[facturaID] = 0;
    totalPorFactura[facturaID] += totalItem;
  });

  console.log("🧾 TOTAL ORIGINAL HTML:", totalOriginalHTML.toFixed(2));
  console.log("📦 SUMA DE ITEMS (data-totalimp):", totalSumadoItems.toFixed(2));
  console.log("📋 DESGLOSE POR FACTURA:", totalPorFactura);
}
function diagnosticarPorFactura() {
  let resumen = {};
  document.querySelectorAll(".factura-card").forEach(factura => {
    const facturaID = factura.dataset.facturaid;
    const totalFacturaSP = parseFloat(
      factura.querySelector(".factura-total").textContent.replace(/\./g,"").replace(",",".")
    );
    let totalItems = 0;
    factura.querySelectorAll(".item-factura").forEach(item => {
      totalItems += parseFloat(item.dataset.totalimp);
    });
    const diff = +(totalItems - totalFacturaSP).toFixed(2);
    resumen[facturaID] = {
      totalFacturaSP: totalFacturaSP,
