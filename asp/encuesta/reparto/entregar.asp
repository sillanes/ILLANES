...
<div class="container my-4">
  ...
  <form id="formPagos" method="post" action="registrar_pago.asp" class="mb-4">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h2>Pedidos</h2> 
    <a href="hojaderuta.asp?hdrid=<%= hojaRutaID %>"  
       class="icon-btn" 
       title="Volver"
...
%>
  ...
  <div class="card mb-3">
    <div class="card-body">
      ...
      <p><strong>Total a Cobrar:</strong> $<span id="totalGeneral"><%If cuentaCorriente=0 THEN %> <%= FormatNumber(ImporteTotal, 2) %> <%Else%> <%= FormatNumber(0.00, 2) %> <%End If%>	 </span></p>
    </div>
  </div>

  <button type="button" class="btn btn-outline-secondary mb-3" onclick="restaurarCantidades()">Restaurar Cantidades</button>
...
<% rs.MoveNext : Loop : rs.Close %>
	  ...
	  %>
		<div class="col-6 col-md-4">
		  ...
		  <input 
			...
			min="0" 
			value="0">
			<!-- <% If cuentaCorriente = 1 Then Response.Write "disabled" %> -->
	...
	<div class="mt-3 p-3 bg-white border rounded">
	  <h5>Saldo restante: $<span id="saldoRestante"><%= FormatNumber(totalCobrar, 2) %></span></h5>
	</div>
...
function actualizarTotales() {
  ...
  if (<%= cuentaCorriente %> == 0) {
    ...
    for (const fid in facturaTotales) {
      const totalSpan = document.querySelector(`.factura-total[data-facturaid="${fid}"]`);
      if (totalSpan) {
        totalSpan.textContent = formatNumberAr(facturaTotales[fid]);
      }
    }
