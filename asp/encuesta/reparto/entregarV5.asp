...
<div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
    ...
    <button type="button"
            class="btn btn-sm btn-danger btn-anular-factura"
...
document.querySelectorAll(".btn-anular-factura").forEach(btn => {
  btn.addEventListener("click", function () {
    ...
    if (!confirm("¿Seguro que querés anular esta factura completa?")) return;

    // Obtener todos los ítems de esta factura
    const items = document.querySelectorAll(
      `.item-factura[data-facturaid="${facturaID}"]`
    );

    items.forEach(item => {
      // Setear cantidad a 0
      item.querySelector(".item-cantidad").textContent = 0;

      // Marcar el input hidden
      item.querySelector(".input-cantidad-item").value = 0;

      // Actualizar el total del ítem
      item.querySelector(".item-total-imp").textContent = "0,00";

      // Estilo visual
      item.classList.add("bg-danger-subtle");
    });

    // Borde rojo al card de factura
    const facturaCard = document.querySelector(
      `.factura-card[data-facturaid="${facturaID}"]`
    );
    if (facturaCard) facturaCard.classList.add("border", "border-danger");
