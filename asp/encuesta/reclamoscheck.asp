...
fromwhat = objRequest("fromwhat")
doWhatPre = objRequest("doWhatPre")
idReclamo = replace(objRequest("idReclamo"),"'","")
ReclamoxControl= replace(objRequest("ReclamoxControl"),"'","")
ResolucionMotivoID= replace(objRequest("ResolucionMotivoID"),"'","")
TransportistaID= replace(objRequest("TransportistaID"),"'","")
ArmadorID= replace(objRequest("ArmadorID"),"'","")
ControladorID= replace(objRequest("ControladorID"),"'","")
AyudanteID= replace(objRequest("AyudanteID"),"'","")
txtresol  = objRequest("txtresolucion") 
txtnotas  = objRequest("textareanotas") 
subirarchivo = objRequest("subirarchivo")
fileupload = objRequest("cmd")
ObservacionesTexto  = objRequest("ObservacionesTexto")
esReclamoxControl  = objRequest("esReclamoxControl")& ""
If ResolucionMotivoID = "" Then
    ResolucionMotivoID = clng("0")
End If
If esReclamoxControl = "" Then
    esReclamoxControl = "0"
End If
If ReclamoxControl = "" Then
    ReclamoxControl = "0"
End If
If doWhat = "-1"  Then 
...
'filesession = Session("FileUploaded")
 ...
 'response.write "<br> fromwhat: " & fromwhat
 'response.write "<br> idReclamo: " & idReclamo
 'response.write "<br> esReclamoxControl: " & esReclamoxControl 
 'response.write "<br> ReclamoxControl: " & ReclamoxControl 
 'response.write "<br> ResolucionMotivoID: " & ResolucionMotivoID 
...
<body>
	...
	<input type="hidden" name="txtnotas" value="<%=txtnotas%>"> 
 	<input type="hidden" name="filesession" value="<%=filesession%>"> 
 	<input type="hidden" name="fromwhat" value="<%=fromwhat%>"> 
...
<%
	...
	if doWhat>="1" and  doWhat<"4"then 
		if doWhat="2" Then
%>		
			<!--#include file="./includes/Reclamos_Notas_upd.asp" -->
<%			
		end If 
		...
		<tr>
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody><tr class="tableHeader">
					<td align="left" colspan="2"><span class="formHeader">Clasificación del Reclamo</span></td>
					</tr>
				</tbody>
			</table>
			</td>
	...
	<tr>
		<td>
	<!--#include file="./includes/Reclamos_Error_Controlador.asp" -->
		</td>
		<td>
	<!--#include file="./includes/Reclamos_Resolucion_Motivos.asp" -->
		</td>
	</tr>
	...
	<tbody>
		<tr>
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody><tr class="tableHeader">
					<td align="left" colspan="1" width="50%"><span class="formHeader">Notas</span></td>
					</tr>
				</tbody>
			</table>
			</td>
	...
	<td>
		<span  class="pagetitle">
			<span>Mis Notas</span> 
			<div align="left">
				<textarea id="textareanotas1" name="textareanotas" style="overflow:auto;resize:none" wrap="hard" rows="12" cols="20" maxlength="500"><%=dbRS("Notas")%></textarea>
			</div> 
	...
	<table border="0" align="center" cellpadding="0" cellspacing="0" class="contentTable table-width-lg" style="width: 100%;margin-bottom: 15px;">
		...
		<tr>
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody><tr class="tableHeader">
					<td align="left" colspan="5"><span class="formHeader">Datos seleccionados</span></td>
					</tr>
				</tbody>
			</table>
			</td>
		...
		<tr align="left" class="columnTop">
			<td height="20" width="20" align="center" ><b>Cliente</b></td>
			<td height="20" width="20" align="center" ><b>Fecha</b></td>
			<td height="20" width="20" align="center" ><b>Nro Factura</b></td>
			<td height="20" width="20" align="center" ><b>Error Control</b></td>
			<td height="20" width="2" align="center"><b>Motivo</b></td>
		</tr>
 

		<tr  align="center" class="itemTD11">
			<td align="center" width="30%"><%=dbRS("Cliente")%></td>
			<td align="center" ><%=dbRS("FechaFactura")%></td>
			<td align="center"> <%=dbRS("FacturaNumero")%></td>
			<td align="center"> <%=dbRS("esReclamoxControl")%></td>
			<td align="center"> <%=dbRS("ResolucionMotivoNombre")%></td>
		</tr>
...
%>	
	...
	<!--#include file="./includes/Reclamos_Notas_upd.asp" -->
	<%if fromwhat=1 Then%>
	<!--#include file="./includes/Reclamos_Clasificacion_upd.asp" -->
	<%End If%>
	...
	<div align="center">
		...
		<input type="button" class="btn1" value="Enviar Mensaje" onClick="chkForm3(this.form, 6, 2,'<%=idReclamo%>')">
...
%>	
	...
	<!--#include file="./includes/Reclamos_Notas_upd.asp" -->
		<form name="FFF2" method="post" action="reclamoscheck.asp">
		<input type="hidden" name="txtresol" value="<%=txtresol%>">     
		
		<input type="hidden" name="esReclamoxControl" value="<%=esReclamoxControl%>"> 
		<input type="hidden" name="ReclamoxControl" value="<%=ReclamoxControl%>"> 
		
		<input type="hidden" name="ResolucionMotivoID" value="<%=ResolucionMotivoID%>"> 
		...
		<input type="hidden" name="doWhat" value="<%=doWhat%>">
		<input type="hidden" name="idReclamo" value="<%=idReclamo%>">
		<input type="hidden" name="doWhatPre" value="<%=4%>">
		 
		<h1>Reclamo: <%=idReclamo%></h1>
	 
		 		
		<!--#include file="./includes/Reclamos_Resolucion_Observacion.asp" --> 
		
		<div align="center">
			<br/> 
			<input type="button" class="btn1" value="Volver" onClick="chkForm3(this.form, 1,3,'<%=idReclamo%>')"> 
			<input type="button" class="btn1" value="Cerrar Reclamo" onClick="chkForm3(this.form, 5,4,'<%=idReclamo%>')">		
		</div>
...
%>	
	...
	<div align="center" class="textSuccess">EL mensaje ha sido enviado al reclamo: <%=idReclamo%> <br/><input type="button" class="btn1" value="Volver" onClick="chkForm(this.form,'')"></div>
