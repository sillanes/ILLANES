...
'response.write "<br> OrdenID: "   & OrdenID
'
'response.write "<br>--------------"
...
<body>
	...
	<form name="FF" method="post" action="imputarordenes.asp">
...
<%
	...
	<!--#include file="./includes/Ordenes_Buscar.asp" --> 
...
%>
		...
		<!--#include file="./includes/OrdenesMostrar.asp" --> 
		<input type="hidden" name="idCliente" value="<%=idCliente%>">
		<input type="hidden" name="idMonto" value="<%=idMonto%>">
		<input type="hidden" name="rango" value="<%=rango%>">
		<input type="hidden" name="txtStartRange" value="<%=txtStartRange%>">
...
%>
		...
		<!--#include file="./includes/OrdenesSave.asp" --> 
<%
		 
		if hasError=0 then
%>
		<span class="textSuccess">Orden: <%=OrdenID%> actualizada</span><br/> 	 
<%
		Else
%>
