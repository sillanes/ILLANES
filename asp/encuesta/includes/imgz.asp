...
<%
  ...
  Dim imgFile, imgWidth, imgHeight, imgDepth, imgType
  Dim objFSO
  
  imgFile = Server.MapPath("/logos/logo-" & Session("pre_UserCompanyID") & ".png")

  Set objFSO = CreateObject("Scripting.FileSystemObject")
  
  If objFSO.FileExists(imgFile) Then
	If gfxSpex(imgFile, imgWidth, imgHeight, imgDepth, imgType) = True Then
		Session("LogoWidth") = imgWidth
	End If
  End If
  ...
  'Dim objFSO, objF, objFC, f1, w, h, c, strType

  'Set objFSO = CreateObject("Scripting.FileSystemObject")
  'Set objF = objFSO.GetFolder("d:\portals\wholesale\xchange\logos")
  'Set objFC = objF.Files

  'response.write "<table border=""0"" cellpadding=""5"">"

  'For Each f1 in objFC
    'if instr(ucase(f1.Name), ".PNG") Or instr(ucase(f1.Name), ".PNG") then
  '	if instr(ucase(f1.Name), "1028") Then
  '     response.write "<tr><td>" & f1.name & "</td><td>" & f1.DateCreated & "</td><td>" & f1.Size & "</td><td>"

   '    if gfxSpex(f1.Path, w, h, c, strType) = true then
   '       response.write f1.Path & " " & w & " x " & h & " " & c & " colors"
   '    else
   '       response.write "&nbsp;"
   '    end if
