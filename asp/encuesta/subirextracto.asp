...
BancoID = clng("0"&objRequest("Bancos") )
...
Dim Form: Set Form = New ASPForm 
  ...
  HTML = HTML & "<form name=""FFUP"" method=post ENCTYPE=multipart/form-data Action=" & ref("UploadID=" & UploadID & "&Action=UPLOAD&doWhat=3&bancos=" & BancoID & "") & " OnSubmit=""return ProgressBar();"">"
  HTML = HTML & "</br>"
...
<body>
	...
	<h1>Subir archivos bancarios</h1>
	 
	<!--#include file="./includes/adm_bancos_combo.asp" -->	

		
	

	<form name="FF" method="post" action="subirextracto.asp">
 ...
 <%
	...
	%>
 		...
 		<%=do_Upload("")%>
		<!--#INCLUDE FILE="_commonExtractos.asp"--> 
		</div>
	<%
	End if
	...
	%>
		<!--#include file="./includes/TMP_ExtractosBancarios_DEL.asp" -->
			
			<%
			If BancoID<>5 Then
			%>
			
			<!--#include file="./includes/TMP_ExtractosBancarios_CSV.asp" -->
			<%	
			Else
			%>
			<!--#include file="./includes/TMP_ExtractosBancarios_XLS.asp" -->
			
			<%
			End If
			%>
			<!--#include file="./includes/ExtractosBancarios_Ins.asp" -->
